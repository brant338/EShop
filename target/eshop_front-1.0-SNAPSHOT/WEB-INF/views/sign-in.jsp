        ...
        <div class="block">
            ...
            <div class="block-body">
                <form>
                    ...
                    <label>密码</label>
                    <input type="password" class="span12" id="pwd" name="pwd" v-model="user.user_pwd"
                           title="ⓘ提示"
                           data-container="body" data-toggle="popover" data-placement="right"
                           data-content="密码不能为空"/>

                    <label>验证码</label>
                    <input type="text" class="span8" id="authCode" name="authCode" v-model="user.authCode"
                           title="ⓘ提示"
                           data-container="body" data-toggle="popover" data-placement="right"
                           data-content="验证码错误"/>
                    ...
                    <a href="javascript:;" class="btn btn-primary pull-right" id="submit1" @click="login()">登录</a>
                    <label class="remember-me"><input type="checkbox" id="rememberme" v-model="user.user_remember">两周内免登录</label>
                    ...
                    axios.post(url, userInfo).then(
                        response => {
                            if (response.data.flag) {
                                this.notifyUrl = response.data.data.notify_url;
                                location.href = this.notifyUrl;
                            } else {
